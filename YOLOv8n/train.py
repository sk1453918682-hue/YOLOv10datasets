...
if __name__ == '__main__':
    model = YOLO(r'E:\yolov8\yolov8n.pt')  # 指定YOLO模型对象，并加载指定配置文件中的模型配置
    # model.load('yolov10s.pt')      #加载预训练的权重文件'yolov10s.pt'，加速训练并提升模型性能
    model.train(data=r'E:\yolov8\ultralytics-main\datasets\kanghuai2\kanghuaixuesuan.yaml',  # 指定训练数据集的配置文件路径，这个.yaml文件包含了数据集的路径和类别信息
                cache=False,  # 是否缓存数据集以加快后续训练速度，False表示不缓存
                imgsz=640,  # 指定训练时使用的图像尺寸，640表示将输入图像调整为640x640像素
                epochs=130,  # 设置训练的总轮数为200轮
